...
`;
		...
		/** Provider balance row (DeepSeek + OpenCode Go), host-resolved keys. */
		function BalanceRow() {
			...
			const load = react.useCallback(async () => {
				...
				const [d, o] = await Promise.all([
					post("/dsh-layout/balance", { type: "deepseek" }),
					post("/dsh-layout/balance", { type: "opencode-go" })
				]);
				setData({ deepseek: d.ok ? d.value : null, opencode: o.ok ? o.value : null, dsErr: d.ok ? null : d.error, ocErr: o.ok ? null : o.error });
				setBusy(false);
			}, []);
			react.useEffect(() => { load(); }, [load]);
			const ds = data?.deepseek;
			const oc = data?.opencode;
			return react_jsx_runtime.jsxs("div", { className: "dsh-layout-balance", children: [
				react_jsx_runtime.jsx("button", { className: "dsh-layout-header-btn", onClick: load, title: "刷新余额/用量", children: busy ? "…" : "⟳" }),
				ds !== null && ds !== void 0
					? react_jsx_runtime.jsx("span", { className: "dsh-layout-balance-item", title: "DeepSeek 官方余额", children: "DS " + ds.balances.map((b) => (b.currency === "CNY" ? "¥" : "$") + fmtNum(b.total)).join(" / ") })
					: data?.dsErr ? react_jsx_runtime.jsx("span", { className: "dsh-layout-balance-err", title: String(data.dsErr.message ?? ""), children: "DS 不可用" }) : null,
				oc !== null && oc !== void 0
					? react_jsx_runtime.jsx("span", { className: "dsh-layout-balance-item", title: "OpenCode Go 用量（滚动/周/月）", children: "OC " + ["rolling", "weekly", "monthly"].map((k) => (oc.usage[k] ? oc.usage[k].percent + "%" : "-")).join("/") })
					: data?.ocErr ? react_jsx_runtime.jsx("span", { className: "dsh-layout-balance-err", title: String(data.ocErr.message ?? ""), children: "OC 不可用" }) : null
			] });
		}

		function FileTree({ cwd }) {
			const [dirCache, setDirCache] = react.useState({});			const [expanded, setExpanded] = react.useState({});
			...
			return react_jsx_runtime.jsxs(react_jsx_runtime.Fragment, { children: [
				...
				react_jsx_runtime.jsx(BalanceRow, {}),
