...
/* Conversation font size, user-adjustable (A−/A+ control); the file tree,
   tool panel and the built-in sidebar keep the official defaults and follow
   the system text scale untouched. Plugin elements are explicitly excluded
   from the conversation rules so the panels never scale with A−/A+. */
[data-conversation-scroll] { font-size: var(--dsh-chat-font-size, 16px); }
[data-conversation-scroll] *:not([class*="dsh-layout"]) { font-size: var(--dsh-chat-font-size, 16px) !important; }
/* Bare (class-less) elements inside plugin surfaces must never inherit the
   conversation size: pin them to the panel root size; classed elements keep
   their own rem hierarchy. */
.dsh-layout-panel *:not([class*="dsh-layout"]),
.dsh-layout-fontctl *:not([class*="dsh-layout"]) { font-size: 0.8125rem !important; }
