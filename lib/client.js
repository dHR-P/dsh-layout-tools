 ...
 *  - font control: conversation font A−/A+ in Settings → plugin config card
...
/* Settings card (settings.plugin.item): conversation-area style settings. */
.dsh-layout-settings-card {
  display: flex; flex-direction: column; gap: 8px;
  padding: 12px 14px;
  border: 1px solid var(--dsw-alias-border-l2); border-radius: 10px;
  background: var(--dsw-alias-bg-base);
}
.dsh-layout-settings-title { font-size: 0.875rem; font-weight: 600; color: var(--dsw-alias-label-primary); }
.dsh-layout-settings-desc { font-size: 0.75rem; color: var(--dsw-alias-label-secondary); }
.dsh-layout-settings-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; }
.dsh-layout-settings-label { font-size: 0.8125rem; color: var(--dsw-alias-label-primary); }
...
/* Left sidebar session rows: the whole row follows the official status dot
   color — green when the conversation ended (done), amber while waiting
   (warning), red on error, blue while running (ongoing). Scoped to the
   official ARIA contract (div[role=treeitem][aria-selected], unique to
   session rows — project rows use aria-expanded) + the dot's own data-state,
   so it survives DSH upgrades without CSS-module hash-coupling. The hover
   card is portalled to <body>, so :has() only sees the row's own dot. */
div[role="treeitem"][aria-selected]:has([data-state="done"]) {
  background: color-mix(in srgb, var(--dsw-alias-state-success-primary) 20%, transparent);
}
div[role="treeitem"][aria-selected]:has([data-state="warning"]) {
  background: color-mix(in srgb, var(--dsw-alias-state-warn-primary) 16%, transparent);
}
div[role="treeitem"][aria-selected]:has([data-state="error"]) {
  background: color-mix(in srgb, var(--dsw-alias-state-error-primary) 16%, transparent);
}
div[role="treeitem"][aria-selected]:has([data-state="ongoing"]) {
  background: color-mix(in srgb, var(--dsw-static-deepseek-450) 12%, transparent);
}
...
`;
		...
		function StatsBar({ sessionId, useSession, useProjection }) {
			...
			const load = react.useCallback(async () => {
				...
				try {
					...
					// Pass the provider identity straight through so the host can
					// resolve the matching key: opencode-go and opencode-go-2 are
					// separate routes with separate keys. "deepseek" keeps its own.
					const kind = prov.toLowerCase().includes("opencode") ? prov
			...
			} else if (balance?.kind === "opencode-go" || (typeof balance?.kind === "string" && balance.kind.startsWith("opencode"))) {
		...
		// ------------------------------------------------------------------
		// Layout settings card (settings.plugin.item): the conversation font
		// control moved off the floating bottom-right seat into the settings
		// plugin-configuration section. Value stays in localStorage + the
		// --dsh-chat-font-size CSS variable (live-apply, no save step).
		// ------------------------------------------------------------------
		function LayoutSettingsCard() {
			...
			return react_jsx_runtime.jsxs("div", { className: "dsh-layout-settings-card", children: [
				react_jsx_runtime.jsx("div", { className: "dsh-layout-settings-title", children: "布局工具" }),
				react_jsx_runtime.jsx("div", { className: "dsh-layout-settings-desc", children: "对话区显示与样式设置" }),
				react_jsx_runtime.jsxs("div", { className: "dsh-layout-settings-row", children: [
					react_jsx_runtime.jsx("span", { className: "dsh-layout-settings-label", children: "对话区字体大小" }),
					react_jsx_runtime.jsxs("div", { className: "dsh-layout-fontctl", children: [
		...
		function apply(ctx) {
			...
			// A full settings page in Settings (settings.section list slot; root
			// scope — no session dependency, so register at the plugin level, not
			// in the session scope below). In rc.7 `settings.plugin.item` became a
			// keyed slot served by host settings namespaces; `settings.section`
			// keeps the plugin's config reachable without host-side wiring.
