...
.dsh-layout-commit-detail {
  ...
  background: var(--dsw-alias-markdown-code-block);
  color: var(--dsw-alias-label-secondary); font-size: 11px; line-height: 1.6;
...
`;
			...
			return react_jsx_runtime.jsxs(react_jsx_runtime.Fragment, { children: [
				...
				branch !== null ? react_jsx_runtime.jsxs("div", { className: "dsh-layout-gitbar", children: [
					react_jsx_runtime.jsx("select", {
						...
						children: branches.map((b) => react_jsx_runtime.jsx("option", { value: b.name, children: b.name + (b.current ? " (当前)" : "") }, b.name))
					}),
		...
		function apply(ctx) {
			...
			ctx.effect(() => {
				...
				// Locate the width-constrained centered column (the only
				// elements with a fixed max-width + margin:0 auto are the
				// conversation content column and the composer/hero cards).
				// Setting the variable ON the reader itself is the only way
				// that reliably wins: inherited values make a naive upward
				// scan land on a descendant where the variable is invisible.
				const findWidthColumn = (from) => {
					let el = from;
					while (el !== null && el !== document.body) {
						const cs = getComputedStyle(el);
						if (cs.maxWidth !== "none" && cs.marginLeft === "auto") return el;
						el = el.parentElement;
					}
					return null;
				};
				const apply = () => {
					const node = document.querySelector("[data-chat-anchor-key]") ?? document.querySelector("[data-conversation-scroll]");
					let target = null;
					if (node !== null) {
						target = findWidthColumn(node);
						if (target === null) {
							// Fallback: the nearest ancestor that declares the
							// variable (an ancestor is always readable).
							let el = node;
							while (el !== null) {
						...
						// hero/blank session: scan #root for the column.
