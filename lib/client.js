...
/* Conversation-bottom status bar: current provider's remaining balance/usage
   (replaces the official turn/step StatsLine). Centered above the composer. */
.dsh-layout-statusbar {
  position: fixed; left: 50%; transform: translateX(-50%);
  bottom: calc(var(--dsh-composer-height, 60px) + 10px);
  z-index: 11;
  display: flex; align-items: center; gap: 6px;
  ...
  padding: 2px 10px;
  box-shadow: 0 2px 8px var(--dsw-alias-bg-mask-2, rgba(0,0,0,.12));
  max-width: min(90vw, 720px); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
}
...
`;
		...
		/** Conversation-bottom status bar: current provider's remaining balance. */
		function BalanceBar({ sessionId }) {
			...
			if (provider === null && balance === null && err === "") return null;
			const text = balance?.kind === "deepseek"
				? "DeepSeek " + balance.balances.map((b) => (b.currency === "CNY" ? "¥" : "$") + fmtNum(b.total)).join(" / ")
				: balance?.kind === "opencode-go"
					? "OpenCode Go " + ["rolling", "weekly", "monthly"].map((k) => (balance.usage[k] ? balance.usage[k].percent + "%" : "-")).join("/")
					: null;
			if (text === null && err === "") return null;
			return react_jsx_runtime.jsxs("div", { className: "dsh-layout-statusbar", children: [
				react_jsx_runtime.jsx("button", { className: "dsh-layout-statusbar-refresh", onClick: load, title: "刷新余额", children: busy ? "…" : "⟳" }),
				err !== ""
					? react_jsx_runtime.jsx("span", { className: "dsh-layout-statusbar-err", title: err, children: "余额不可用" })
					: react_jsx_runtime.jsx("span", { className: "dsh-layout-statusbar-text", title: "当前模型来源：" + (provider ?? ""), children: text })
			] });
		}
		...
		function Panels({ sessionId, useSession, useSessions }) {
			...
			return react_jsx_runtime.jsxs(react_jsx_runtime.Fragment, { children: [
				...
				react_jsx_runtime.jsx(BalanceBar, { sessionId })
			] });
		}
