		...
		function apply(ctx) {
			...
			ctx.effect(() => {
				...
				const apply = () => {
					...
					// No chat anchor (hero/blank session): fall back to a
					// document-wide scan for the variable owner — cheap while
					// the hero tree is small.
					const owner = (() => {
						if (node !== null) {
							let el = node;
							while (el !== null) {
								const v = getComputedStyle(el).getPropertyValue("--dsh-chat-content-width");
								if (v !== undefined && v.trim() !== "") return el;
								el = el.parentElement;
							}
							return null;
						}
						...
						for (let i = 0; i < all.length; i++) {
							const v = getComputedStyle(all[i]).getPropertyValue("--dsh-chat-content-width");
							if (v !== undefined && v.trim() !== "") return all[i];
						}
						return null;
					})();
					if (owner === null) {
					...
					if (owner !== currentRoot) {
						currentRoot = owner;
						owner.style.setProperty("--dsh-chat-content-width", CHAT_WIDTH);
						columnEl = findColumn(owner);
					}
					syncMargins();
				};
				apply();
