...
/** API balance/usage endpoints (provider balance display). */
const DSH_HOME = join(process.env.USERPROFILE || process.env.HOME || '', '.dsh');
const BALANCE_TIMEOUT_MS = 15000;

/** Read `KEY: value` pairs from ~/.dsh/.credentials.yaml (values never leave the host). */
...
/** Query one provider's balance/usage (keys resolved host-side only). */
async function balanceFor(type) {
	const creds = await readCredentials();
	if (type === 'deepseek') {
		const key = creds.DEEPSEEK_API_KEY;
		if (!key) return { ok: false, error: { code: 'no-key', message: 'DEEPSEEK_API_KEY 未配置' } };
		try {
			const res = await fetch('https://api.deepseek.com/user/balance', {
				headers: { Authorization: `Bearer ${key}`, Accept: 'application/json' },
				signal: AbortSignal.timeout(BALANCE_TIMEOUT_MS)
			});
			if (!res.ok) return { ok: false, error: { code: 'http', message: `HTTP ${res.status}` } };
	...
	// OpenCode family: opencode-go and opencode-go-2 are separate routes with
	// separate keys. Resolve the key by provider name so the shown usage tracks
	// the active opencode route.
	if (typeof type === 'string' && /^opencode/i.test(type)) {
		// Explicit provider -> key mapping (the `_2` suffix lands after API_KEY).
		const keyEnv = type === 'opencode-go-2' ? 'OPENCODE_GO_API_KEY_2' : 'OPENCODE_GO_RESPONSES_API_KEY';
		const key = creds[keyEnv] || creds[type === 'opencode-go' ? 'OPENCODE_GO_API_KEY' : keyEnv];
		if (!key) return { ok: false, error: { code: 'no-key', message: `${type} 密钥未配置` } };
		try {
			...
			for (const k of ['rolling', 'weekly', 'monthly']) {
				if (u[k] && typeof u[k] === 'object') {
					usage[k] = { status: u[k].status ?? '', percent: u[k].percent ?? 0, resetsAt: u[k].resetsAt ?? '' };
				}
			}
			return { ok: true, value: { kind: type, usage } };
		} catch (error) {
			return { ok: false, error: { code: 'network', message: String(error?.message ?? error) } };
		}
	}
	return { ok: false, error: { code: 'bad-request', message: 'unknown balance type' } };
}
...
async function gitBranches(runner, canonical) {
	...
	// Empty repository (no commits yet): no refs exist, but HEAD's symbolic
	// branch is real — surface it so the picker is never empty.
	if (branches.length === 0 && head.branch !== '') {
		branches.push({ name: head.branch, current: true });
	}
...
/**
 * Ensure the workspace-root layer itself is a git repository — strictly the
 * path's own layer, never upward: if `.git` exists here (file or directory,
 * e.g. linked worktrees) it is used as-is; otherwise `git init` creates one
 * right here. Nested folders and parent directories are never inspected.
 * `git init` never touches working files.
 */
async function ensureRepo(runner, canonical) {
...
/**
 * Open a file with the OS default association — exactly like double-clicking
 * it in Explorer: `cmd /c start` performs a Windows ShellExecute, so .txt
 * opens in Notepad, .md/.js in VS Code when the user associated them, etc.
 * The raw path is passed unquoted: Node quotes argv entries that need it
 * (spaces), and cmd's `start` treats its first quoted argument as a title.
 */
function openWithDefaultApp(canonical) {
	return new Promise((resolve) => {
		const child = spawn('cmd.exe', ['/c', 'start', '', canonical], {
			stdio: 'ignore',
			windowsHide: true
		});
		child.once('error', () => resolve({ ok: false, error: { code: 'open-failed', message: 'failed to launch the default application' } }));
		child.once('exit', (code) => resolve(code === 0 ? { ok: true } : { ok: false, error: { code: 'open-failed', message: `shell exited with code ${code}` } }));
	});
}
...
function registerRoutes(ctx) {
	...
	const handler = async (req, res) => {
		...
		// Balance route needs no workspace path — serve it before the gate.
		if (pathname === '/dsh-layout/balance') {
			const type = typeof payload === 'object' && payload !== null ? payload.type : void 0;
			const result = await balanceFor(type);
		...
		try {
			// Git routes: make sure the workspace-root layer itself is a
			// repository (auto-init right there when .git is absent).
			if (pathname.startsWith('/dsh-layout/git-')) {
				await ensureRepo(runner, allowed.canonical);
			}
			switch (pathname) {
				...
				case '/dsh-layout/open-file': {
					const info = await stat(allowed.canonical);
					if (!info.isFile()) {
						json(res, FAIL({ code: 'not-a-file', message: 'path is not a file' }));
						return;
					}
					json(res, OK(await openWithDefaultApp(allowed.canonical)));
					return;
				}
				default:
					res.writeHead(404);
