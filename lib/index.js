...
async function gitBranches(runner, canonical) {
	const head = await runner.run(['rev-parse', '--abbrev-ref', 'HEAD'], canonical);
	if (head.exitCode !== 0) return { root: null, branch: null, branches: [] };
	...
	return { root: canonical, branch: head.stdout.trim(), branches };
}
...
async function gitLog(runner, canonical, limit) {
	const head = await runner.run(['rev-parse', '--abbrev-ref', 'HEAD'], canonical);
	if (head.exitCode !== 0) return { root: null, branch: null, commits: [] };
	...
	return { root: canonical, branch: head.stdout.trim(), commits };
}
