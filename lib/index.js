...
async function balanceFor(type) {
	...
	if (type === 'opencode-go') {
		const key = creds.OPENCODE_GO_RESPONSES_API_KEY || creds.OPENCODE_GO_API_KEY;
		if (!key) return { ok: false, error: { code: 'no-key', message: 'OpenCode Go 密钥未配置' } };
		try {
			...
			return { ok: true, value: { kind: 'opencode-go', usage } };
		} catch (error) {
			return { ok: false, error: { code: 'network', message: String(error?.message ?? error) } };
		}
	}
