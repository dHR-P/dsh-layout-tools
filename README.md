...
- **侧边栏会话状态着色**：左侧工作区列表里，会话条目**整行跟随官方状态点着色**——对话结束显示绿底（官方绿点）、等待回复黄底（官方黄点）、运行中蓝底、出错红底；仅作用于官方会话行本身的范围（跟随官方行的尺寸与圆角）
- **底部状态栏**：对话区底部（**输入框下方**，与官方 StatsLine 同位置，通过 portal 渲染进官方容器替换）显示一行统计——**当前所选模型来源的余额/用量替换了"轮/步"计数**（选中 DeepSeek 官方显示 `¥` 余额、OpenCode Go 显示滚动/周/月用量），**耗时 / 速度 / token 统计保留**（与官方 StatsLine 同数据源）；点击 `⟳` 刷新余额（密钥仅 host 侧从 `~/.dsh/.credentials.yaml` 读取）
