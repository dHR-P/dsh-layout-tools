...
- **底部状态栏**：对话区底部（输入框上方）显示一行统计——**当前所选模型来源的余额/用量替换了"轮/步"计数**（选中 DeepSeek 官方显示 `¥` 余额、OpenCode Go 显示滚动/周/月用量），**耗时 / 速度 / token 统计保留**（与官方 StatsLine 同数据源）；点击 `⟳` 刷新余额（密钥仅 host 侧从 `~/.dsh/.credentials.yaml` 读取）
